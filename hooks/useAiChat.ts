...
export function useAiChat(sessionId: string) {
  ...
  // sessionId+model 별로 buildHarness 보장 호출 했는지 — agent 모드 첫 호출 전 1회만.
  ...
  // build promise 추적 — 동일 세션+모델에서 첫 send 가 진행 중일 때 두번째 send 가 동시에 build 시도하는 케이스 차단.
  const harnessBuildInFlightRef = useRef<Map<string, Promise<boolean>>>(new Map());
  ...
  useEffect(() => {
    return () => {
      ...
      // harness ensure flag 는 sessionId+model 별. 세션 변경 시 현재 세션 관련 캐시/진행 빌드만 정리한다.
      const prefix = `${sessionId}:`;
      harnessBuildEnsuredRef.current.forEach((key) => {
        if (key.startsWith(prefix)) harnessBuildEnsuredRef.current.delete(key);
      });
      harnessBuildInFlightRef.current.forEach((_, key) => {
        if (key.startsWith(prefix)) harnessBuildInFlightRef.current.delete(key);
      });
    };
  }, [sessionId]);
  ...
  const send = useCallback(async (
    ...
    if (isBackendSessionId(sessionId)) {
      ...
      if (mode === "agent") {
        ...
        //
        //   해결: sessionId+model 별로 첫 agent send 직전에 buildHarness 1회 보장 호출. idempotent —
        //   이미 build 된 세션이면 AI 가 재컴파일만 함. 동시 send 가 일어나면 in-flight promise 공유.
        const buildModel = modelName ?? "GPT_5_MINI";
        const harnessBuildKey = `${sessionId}:${buildModel}`;
        if (isBackendSessionId(sessionId) && !harnessBuildEnsuredRef.current.has(harnessBuildKey)) {
          let inflight = harnessBuildInFlightRef.current.get(harnessBuildKey);
          if (!inflight) {
            inflight = sessionApi.buildHarness(sessionId, buildModel)
              .then(() => true)
              .catch((err) => {
                ...
                harnessBuildEnsuredRef.current.delete(harnessBuildKey);
                return false;
              });
            harnessBuildInFlightRef.current.set(harnessBuildKey, inflight);
          }
          const buildEnsured = await inflight;
          harnessBuildInFlightRef.current.delete(harnessBuildKey);
          if (buildEnsured) harnessBuildEnsuredRef.current.add(harnessBuildKey);
        }
